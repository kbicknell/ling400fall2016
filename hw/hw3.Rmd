...
3. **Indexing.** Index the wikipedia pages for countries and their capitals. [This page](https://en.wikipedia.org/wiki/List_of_national_capitals_in_alphabetical_order) contains links to every country and its capital.
    ...
    * For specific examples of adding documents to a Lucene index, see [the Lucene tutorial slides](http://www.klintonbicknell.com/ling400fall2016/dl/ling400_class5c.pdf) from class a couple of weeks ago and [the Lucene in Action](http://www.manning.com/hatcher3/) source code download. Of course, [the Lucene javadocs](http://lucene.apache.org/core/6_2_1/core/index.html) will also be very useful.
    * There's no need to use any fancy html parsing libraries for this assignment, but you will need to strip out the html from the web pages before sending them on to an `EnglishAnalyzer` and indexing them. One convenient way to do this is to use the [`HTMLStripCharFilter`](https://lucene.apache.org/core/6_2_1/analyzers-common/org/apache/lucene/analysis/charfilter/HTMLStripCharFilter.html) class.
...
hrase-query.html) a useful reference.
