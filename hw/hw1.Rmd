...
**Sentence splitting** To split the file into sentences, you should use [Stanford CoreNLP](http://stanfordnlp.github.io/CoreNLP/), to become familiar with the package, since we will be using it for later assignments. CoreNLP is Java software, so you may use it natively from Java. To access it from within Python, you'll need to use a wrapper. I recommend [the one](https://github.com/smilli/py-corenlp), which requires that you're running the corenlp server.
